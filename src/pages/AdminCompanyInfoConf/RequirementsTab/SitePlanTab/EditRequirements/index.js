...
const EditRequirements = ({ setEditModal, editModal, toEdit }) => {
  ...
  const { changeTextRequirement, changeReferenceRequirement, changeRequestedRequirement } = bindActionCreators(
    actions,
    dispatch,
  );
  const onFinish = ({ reference, requested, wysiwyg }) => {
    ...
    if (requested !== undefined) changeRequestedRequirement({ key: toEdit.key, note: toEdit.note, text: requested });
    form.resetFields();
    setEditModal(() => false);
  };
